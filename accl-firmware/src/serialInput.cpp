...
char *readSerial()
{
  ...
  char *input[] = {"{\"name\":\"NO_NAME\",\"curve_type\":\"Linear\",\"loop",
                   "s\":false,\"points\":[{\"time\":0.0,\"current\":0.", "0},{\"time\":1.0,\"current\":432.0}]}"};
...
typedef struct extractResult ExtractResult;
ExtractResult extractField(char *buffer)
{
  ...
  return ExtractResult{
    field : RoutineField::NOT_A_FIELD,
    intValue : NULL,
    strValue : NULL
  };
}
...
void extractData(char *buffer, Routine *result)
{
  ...
  if (value[0] == '"')
  {
    extracted = extractField(value);
    // Serial.print(extracted.field);
    // Serial.println(extracted.strValue);
    // if (extracted.field == RoutineField::NAME)
    //   strcpy(result.name, extracted.strValue);
    // else if (extracted.field == RoutineField::CURVE_TYPE)
    //   result.curveType = (CurveType)extracted.intValue;
  }
  else
  {
    strcpy(buffer, strstr(buffer, ","));
    extractData(buffer, result);
  }
  // } while (extracted.field != RoutineField::NOT_A_FIELD);
}
