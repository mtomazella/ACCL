...
char *readSerial()
{
  ...
  char *input[] = {"{\"name\":\"cuuuu zinho\",\"curve_type\":\"Monotone\""}; //,\"loop",
  // "s\":true"
  // }; //,\"points\":[{\"time\":0.0,\"current\":0.", "0},{\"time\":1.0,\"current\":432.0}]}"};
...
void handleSerialInput(char **buffer, Routine *routine)
{
  ...
  Serial.print(routine->name);
  Serial.print(" ");
  Serial.print((int)routine->curveType);
  Serial.print(" ");
  Serial.println(routine->loop);
